...
in {
  ...
  larstop2 = ShellApplicationNoCheck {
    ...
    text = ''
      sudo nixos-rebuild switch --update-input my-modules --commit-lock-file  --flake ${selfLocation} $@
    '';
  };
  muehml = ShellApplicationNoCheck {
    ...
    runtimeInputs = [nixos-rebuild];
    text = ''
      nixos-rebuild switch --target-host root@muehml.eu --update-input my-modules --commit-lock-file --flake ${selfLocation}#muehml
    '';
  };
  htpc = ShellApplicationNoCheck {
    ...
    runtimeInputs = [nixos-rebuild];
    text = ''
      nixos-rebuild switch --target-host root@htpc.local --update-input my-modules --commit-lock-file --flake ${selfLocation}#htpc
    '';
  };
}
